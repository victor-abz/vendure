...
<MemberInfo kind="method" type={`() => Array<{ key: string; config: <a href='/reference/typescript-api/settings-store/settings-store-field-config#settingsstorefieldconfig'>SettingsStoreFieldConfig</a> }>`}  since="3.6.0"  />

Returns all registered field definitions with their full keys.
### getScopeType

<MemberInfo kind="method" type={`(fieldConfig: <a href='/reference/typescript-api/settings-store/settings-store-field-config#settingsstorefieldconfig'>SettingsStoreFieldConfig</a>) => SettingsStoreScopeType`}  since="3.6.0"  />
