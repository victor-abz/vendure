...
* `optionGroups`: OptionGroups define what variants make up the product. Applies only to products with more than one variant. By default, each product gets its own option groups. To **share an option group** across multiple products (e.g. a common "Size" group for all shoes), use the `name:code` syntax: `size:shoe-size`. All products that reference the same code will share a single option group. Products using a plain name (e.g. just `size`) will each get their own independent option group. For example:
  ```csv
