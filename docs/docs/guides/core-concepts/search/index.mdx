...
The [`@vendure-community/elasticsearch-plugin`](https://github.com/vendurehq/vendure-community-plugins) stores the search index in an Elasticsearch cluster. It is designed for production workloads with large catalogs where you need more advanced full-text search capabilities, better performance at scale, or features like fuzzy matching and relevance tuning.
