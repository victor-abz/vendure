...
The default `apiOptions.cors` value is `{ origin: true, credentials: true }`. `origin: true` reflects
the `Origin` header of any caller back as `Access-Control-Allow-Origin`, so any website can make
cross-origin requests to your Shop and Admin APIs. Replace it with the origins you actually serve:
...
Vendure logs a warning at startup if it finds the reflect-any-origin default in place. The warning is
most important when you also set `authOptions.cookieOptions.sameSite` to `'none'` or when you use
`tokenMethod: 'bearer'`, because neither of those relies on the `SameSite` cookie rules that otherwise
limit the impact. See the [Security guide](/developer-guide/security/#configure-cors) for the
full explanation.
