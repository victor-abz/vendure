...
:::info
The one-click-deploy repo already contains S3-compatible asset storage configuration. If you are using that repo, check the existing environment variable names before replacing the snippet below.
:::
...
export const config: VendureConfig = {
    ...
    plugins: [
        AssetServerPlugin.init({
            ...
            storageStrategyFactory: configureS3AssetStorage({ // [!code highlight]
                bucket: process.env.SPACES_BUCKET || 'vendure-assets', // [!code highlight]
                credentials: { // [!code highlight]
                    accessKeyId: process.env.SPACES_ACCESS_KEY_ID!, // [!code highlight]
                    secretAccessKey: process.env.SPACES_SECRET_ACCESS_KEY!, // [!code highlight]
                }, // [!code highlight]
                nativeS3Configuration: { // [!code highlight]
                    endpoint: process.env.SPACES_ENDPOINT!, // [!code highlight]
                    signatureVersion: 'v4', // [!code highlight]
                    region: process.env.SPACES_REGION || 'nyc3', // [!code highlight]
                }, // [!code highlight]
            }), // [!code highlight]
        }),
    ],
...
SPACES_BUCKET=vendure-assets
SPACES_REGION=<use the region of your Space, e.g. nyc3> # [!code highlight]
SPACES_ENDPOINT=<use the endpoint of your Spaces region, e.g. https://nyc3.digitaloceanspaces.com> # [!code highlight]
SPACES_ACCESS_KEY_ID=<use the key generated earlier> # [!code highlight]
SPACES_SECRET_ACCESS_KEY=<use the secret generated earlier> # [!code highlight]
```
